 ...
 Class Cs_Group_View extends Cs_View  {
    ...
    */
	...
	public function display() : string {
        ...
        $output = '<div'
					. ( ( $this->cs_group->is_identifier() ) ? ' id="b4cs-group-' . $this->cs_group->get_identifier() . '"' : '' )
					. ' class="b4cs-card b4cs-group">' . "\n";

		// Display the image area
		$output .= '  <div class="b4cs-group-image-area">' . "\n";
		$output .= '    ' . $this->cs_group->get_image_URL() . "\n";
		$output .= '  </div>' . "\n";
		
		// Display the details area
        $output .= '  <div class="b4cs-group-details-area">' . "\n";

		// Display the group name in a link if a link is provided
		$output .= '  <div class="b4cs-group-name">' .
					( ( $this->cs_group->is_URL() ) ? '<a class="b4cs-group-link" target="_blank" href="' . $this->cs_group->get_URL( $this->cs ) . '">' : '') .
        ...
        $output .= ( $this->cs_group->is_location() ) ? '    <div class="b4cs-location"><span class="b4cs-location-gliph">' . $this->cs_group->get_location() . '</span></div>' . "\n" : '';

        $output .= ( $this->cs_group->is_time_of_meeting() ) ? '    <div class="b4cs-time"><span class="b4cs-time-gliph">' . $this->cs_group->get_time_of_meeting() . '</span></div>' . "\n" : '';

		// Display the description of the group
        $output .= ( $this->cs_group->is_description() ) ? '    <div class="b4cs-description">' . $this->cs_group->get_description() . '</div>' . "\n" : '';
