...
use amb_dev\b4cs\Cs_Item as Cs_Item;
 ...
 */
 class Cs_Event extends Cs_Item {
	...
	protected function sanitize_status( \stdclass $event_obj ) : string {
		...
		$result = \amb_dev\b4cs\Cs_Event::EVENT_STATUS[0];
		if ( isset( $event_obj->status ) ) {
			$result = strtolower( $event_obj->status );
			$result = ( in_array( $result, \amb_dev\b4cs\Cs_Event::EVENT_STATUS ) ) ? $result : \amb_dev\b4cs\Cs_Event::EVENT_STATUS[0];
		}
		return $result;
	}
