...
final class ChurchSuite {
	...
	public function __construct( string $church_name, string $JSON_base = ChurchSuite::EVENTS ) {
		$this->church_name = self::sanitize_church_name( $church_name );
		$this->JSON_base = self::sanitize_JSON_base( $JSON_base );
	}
	...
	private static function sanitize_alpha_only( string $str ) : string {
		return ( isset( $str ) ) ? strtolower( preg_replace( "/[^a-zA-Z]+/", "", $str ) ) : '';
	}

	/*
	 * Helper function to sanitize the church name to contain only A-Za-z, changed to lower case
	 * or, if there is no church_name, to replace it with 'demo' so we get some result from the query
	 * 
	 * @since 1.0.0
	 * @param	string	any string value
	 * @return	string	the supplied string sanitized to a-zA-Z and then transformed to lowercase
	 */
	public static function sanitize_church_name( string $church_name ) : string {
		$church_name = self::sanitize_alpha_only( $church_name );
		return ( $church_name === '' ) ? "demo" : $church_name;
	}

	/*
	 * Helper function to sanitize the JSON_base value to be one of the permissible value
	 * or to return the default value: ChurchSuite::EVENTS
	 * 
	 * @since 1.0.0
	 * @param	string	any string value
	 * @return	string	the supplied string sanitized to a permitted JSON_base value
	 */
	public static function sanitize_JSON_base( string $JSON_base ) : string {
		$JSON_base = self::sanitize_alpha_only( $JSON_base );
		return ( ( $JSON_base !== '' ) && array_key_exists( $JSON_base, ChurchSuite::BASE_JSON_URLS ) ) ? $JSON_base : ChurchSuite::EVENTS;
	}
